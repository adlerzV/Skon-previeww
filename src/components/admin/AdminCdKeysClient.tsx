...
export default function AdminCdKeysClient({ initial }: { initial?: Data }) {
  ...
  return <AdminPage>
    ...
    {permissions.includes("cdkeys.write") && <AdminCard className="mb-4 p-4">
      <div className="flex items-start gap-3">
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[5px] bg-brand-blue/10 text-brand-blue"><UploadCloud size={17}/></span>
        <div><h2 className="text-sm font-black text-white">افزودن موجودی</h2><p className="mt-1 text-[10px] leading-5 text-brand-m_khonsa">شناسه محصول و تنوع را وارد کن و هر کلید را در یک خط قرار بده.</p></div>
      </div>
      <div className="mt-4 grid gap-2 md:grid-cols-2"><input value={productId} onChange={e=>setProductId(e.target.value)} placeholder="شناسه محصول" className="w-full min-h-10 rounded-[5px] border border-brand-surface_hover bg-brand-bg px-3 text-xs text-white outline-none placeholder:text-brand-m_khonsa focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/20" dir="ltr"/><input value={variationId} onChange={e=>setVariationId(e.target.value)} placeholder="شناسه تنوع" className="w-full min-h-10 rounded-[5px] border border-brand-surface_hover bg-brand-bg px-3 text-xs text-white outline-none placeholder:text-brand-m_khonsa focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/20" dir="ltr"/></div>
      <textarea value={keys} onChange={e=>setKeys(e.target.value)} rows={5} placeholder="هر CD Key در یک خط" className="w-full min-h-10 rounded-[5px] border border-brand-surface_hover bg-brand-bg px-3 text-xs text-white outline-none placeholder:text-brand-m_khonsa focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/20 mt-2" dir="ltr"/>
      <div className="mt-3 flex flex-wrap items-center gap-2"><button onClick={importKeys} disabled={loading || !productId || !variationId || !keys.trim()} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-[5px] px-3 text-xs font-black transition disabled:pointer-events-none disabled:opacity-50 bg-brand-blue text-white hover:brightness-110">افزودن به موجودی</button>{message&&<span className="text-[10px] text-brand-m_khonsa">{message}</span>}</div>
    </AdminCard>}

    <AdminCard className="overflow-hidden">
      <div className="flex flex-col gap-2 border-b border-white/[.06] p-3 sm:flex-row"><select value={status} onChange={e=>setStatus(e.target.value)} className="w-full min-h-10 rounded-[5px] border border-brand-surface_hover bg-brand-bg px-3 text-xs text-white outline-none placeholder:text-brand-m_khonsa focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/20 sm:max-w-[220px]"><option value="all">همه وضعیت‌ها</option><option value="available">موجود</option><option value="reserved">رزروشده</option><option value="used">مصرف‌شده</option><option value="duplicate">تکراری</option><option value="decrypt_failed">خطای رمزگشایی</option></select><button onClick={()=>void load()} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-[5px] px-3 text-xs font-black transition disabled:pointer-events-none disabled:opacity-50 bg-brand-blue text-white hover:brightness-110">اعمال فیلتر</button></div>
      <div className="hidden lg:block overflow-x-auto"><table className="w-full border-collapse text-right [&_th]:whitespace-nowrap [&_th]:border-b [&_th]:border-white/[.06] [&_th]:px-3.5 [&_th]:py-3 [&_th]:text-[9px] [&_th]:font-black [&_th]:text-brand-m_khonsa [&_td]:border-b [&_td]:border-white/[.06] [&_td]:px-3.5 [&_td]:py-3 [&_td]:align-middle [&_tr:last-child_td]:border-b-0 [&_tbody_tr]:transition-colors [&_tbody_tr:hover]:bg-white/[.02] min-w-[980px]"><thead><tr><th>شناسه</th><th>محصول</th><th>تنوع</th><th>وضعیت</th><th>سفارش</th><th>تاریخ</th><th>تلاش تخصیص</th></tr></thead><tbody>{data.nodes.map(row=><tr key={row.stockId}><td className="text-[10px] text-white" dir="ltr">#{row.stockId}</td><td><div className="text-[10px] font-bold text-white">{row.productName}</div><div className="mt-1 text-[9px] text-brand-m_khonsa" dir="ltr">{row.productId}</div></td><td><div className="text-[10px] text-white">{row.variationName}</div><div className="mt-1 text-[9px] text-brand-m_khonsa" dir="ltr">{row.variationId}</div></td><td><AdminBadge tone={row.status==="available"?"success":row.status==="failed"||row.status==="decrypt_failed"?"danger":"neutral"}>{LABELS[row.status]??row.status}</AdminBadge></td><td className="text-[10px] text-white" dir="ltr">{row.orderId?`#${row.orderId}`:"—"}</td><td className="text-[9px] text-brand-m_khonsa">{row.createdAt?new Date(row.createdAt).toLocaleDateString("fa-IR"):"—"}</td><td className="text-[10px] text-white" dir="ltr">{row.assignmentAttempts}</td></tr>)}</tbody></table></div>
