...
export default function AdminCustomersClient({initial}:{initial?:{nodes:Customer[];pageInfo:{hasNextPage:boolean;endCursor:string|null}}}){
  ...
  return <AdminPage>
    ...
    {data.nodes.length===0?<AdminEmpty title="مشتری‌ای با این جستجو پیدا نشد."/>:<>
      <AdminTable minWidth="920px"><thead><tr><th>مشتری</th><th>ایمیل</th><th>سفارش</th><th>تیکت</th><th>دیدگاه</th><th>ثبت‌نام</th><th/></tr></thead><tbody>{data.nodes.map(c=><tr key={c.databaseId}><td><div className="flex items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-[6px] bg-white/[.04] text-brand-m_khonsa"><Users size={15}/></span><div><div className="text-xs font-black text-white">{c.name||"—"}</div><div className="mt-1 flex items-center gap-1 text-[9px] text-brand-m_khonsa">#{c.databaseId}{c.isStaff&&<AdminBadge tone="info">مدیر</AdminBadge>}</div></div></div></td><td><span className="text-[10px] text-white" dir="ltr">{c.email}</span></td><td className="text-[11px] font-bold text-white">{c.ordersCount.toLocaleString("fa-IR")}</td><td className="text-[11px] font-bold text-white">{c.ticketsCount.toLocaleString("fa-IR")}</td><td className="text-[11px] font-bold text-white">{c.reviewsCount.toLocaleString("fa-IR")}</td><td><span className="text-[9px] text-brand-m_khonsa">{c.registeredAt?new Date(c.registeredAt).toLocaleDateString("fa-IR"):"—"}</span></td><td><Link href={`/admin/customers/${c.databaseId}`} className="inline-flex items-center gap-1 text-[10px] font-black text-brand-blue hover:text-white"><Eye size={14}/> پروفایل</Link></td></tr>)}</tbody></AdminTable>
