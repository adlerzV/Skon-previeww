            ...
            <div className="max-h-[420px] overflow-y-auto">
              {items.slice(0, 20).map((notification) => (
                <div key={notification.databaseId} className={`p-3 border-b border-brand-surface_hover ${notification.isRead ? "" : "bg-brand-blue/5"}`}>
                  {notification.link ? (
                    <Link href={notification.link} onClick={() => setOpen(false)} className="block">
              ...
              {items.length === 0 && <div className="p-8 text-center text-xs text-brand-m_khonsa">اعلانی برای نمایش وجود ندارد.</div>}
            </div>
          </div>
        </>
      )}
