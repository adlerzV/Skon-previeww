...
export default function AdminAuditClient(){
 ...
 const refresh=useCallback(async()=>{setLoading(true);setError("");try{const q=result?`?result=${encodeURIComponent(result)}`:"";const r=await fetch(`/api/admin/audit${q}`,{cache:"no-store"});const body=await r.json();if(!r.ok)throw new Error(body?.error||"خطا در دریافت گزارش حسابرسی");setItems(Array.isArray(body)?body:[])}catch(e){setError(e instanceof Error?e.message:"خطا در دریافت گزارش حسابرسی")}finally{setLoading(false)}},[result]);
 useEffect(()=>{void refresh()},[refresh]);
 return <AdminPage className="max-w-[1280px]">
  <AdminPageIntro eyebrow="فاز ۵ · حسابرسی" title="گزارش حسابرسی" description="عملیات حساس پنل و موتور سیستم با نتیجه و شناسه مدیر ثبت می‌شوند." action={<AdminRefreshButton onClick={()=>void refresh()} loading={loading}/>}/>
