...
export default function AdminCustomerDetail({data:initialData,id}:{data?:any;id:number}){
  ...
  if(!data) return <AdminPage><AdminBackLink href="/admin/customers"><ArrowRight size={15}/> بازگشت به مشتریان</AdminBackLink>{error&&<div className="mt-4 rounded-[7px] border border-red-500/20 bg-red-500/5 p-4 text-xs text-red-200">{error}</div>}<div className="mt-4"><AdminEmpty title={loading?"پروفایل در حال دریافت اطلاعات...":"کاربر پیدا نشد."}/></div></AdminPage>;
  ...
  return <AdminPage>
    ...
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-[360px_1fr]">
      <AdminCard className="overflow-hidden"><AdminSectionHeader icon={<UserRound size={17}/>} title="اطلاعات حساب" description="مشخصات پایه و آمار فعالیت مشتری"/><div className="p-4"><AdminFieldRow label="ایمیل" value={c.email||"—"}/><AdminFieldRow label="ثبت‌نام" value={c.registeredAt?new Date(c.registeredAt).toLocaleString("fa-IR"):"—"}/><AdminFieldRow label="سفارش‌ها" value={c.ordersCount.toLocaleString("fa-IR")} ltr={false}/><AdminFieldRow label="تیکت‌ها" value={c.ticketsCount.toLocaleString("fa-IR")} ltr={false}/><AdminFieldRow label="دیدگاه‌ها" value={c.reviewsCount.toLocaleString("fa-IR")} ltr={false}/><div className="mt-4 rounded-[6px] bg-white/[.025] p-3 text-[10px] leading-5 text-brand-m_khonsa">اطلاعات احراز هویت و فیلدهای امنیتی در این صفحه نمایش داده نمی‌شوند.</div></div></AdminCard>
