...
export default function AdminEngineClient() {
  ...
  const stats = useMemo(() => health ? [
    ["زمان‌بندی در انتظار", health.actionSchedulerPending, health.actionSchedulerOverdue ? `${health.actionSchedulerOverdue} مورد عقب‌افتاده` : ""],
    ["کارهای ناموفق", health.actionSchedulerFailed, "هوک‌های BTL"],
    ["پاک‌سازی کش", health.revalidationPending + health.revalidationClaimed, `${health.revalidationFailed} ناموفق`],
    ["رویدادهای حسابرسی", health.auditEntries, health.auditTableReady ? "جدول آماده" : "جدول موجود نیست"],
  ] as const : [], [health]);

  return <AdminPage className="max-w-[1280px]">
    <AdminPageIntro eyebrow="فاز ۵ · موتور سیستم" title="وضعیت موتور و زیرساخت" description="قیمت‌گذاری، نرخ ارز، زمان‌بندی، پاک‌سازی کش، سلامت سیستم و کارهای ناموفق را از یک نقطه کنترل کن." action={<AdminRefreshButton onClick={() => void refresh()} loading={loading} />} />
    ...
    {!health && loading ? <AdminEmpty title="وضعیت Engine در حال دریافت است..." /> : !health ? <AdminEmpty title="وضعیت Engine در دسترس نیست." description={error || "دوباره بروزرسانی را بزن."} /> : <>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <AdminCard className="p-5"><div className="flex items-center justify-between gap-3"><div><div className="admin-eyebrow">اجرا و نسخه‌ها</div><h2 className="mt-1 text-base font-black text-white">نسخه و سرویس‌ها</h2></div><Server size={19} className="text-brand-blue" /></div><div className="mt-5 grid grid-cols-2 gap-3 text-xs">{[["BTL Engine", health.pluginVersion],["Schema", health.schemaVersion],["PHP", health.phpVersion],["WordPress", health.wordpressVersion],["WooCommerce", health.woocommerceVersion || "—"],["WPGraphQL", health.graphqlVersion || "—"]].map(([k,v]) => <div key={k} className="rounded-xl border border-white/[.06] bg-white/[.02] p-3"><div className="text-[10px] text-brand-m_khonsa">{k}</div><div className="mt-1 font-black text-white" dir="ltr">{v}</div></div>)}</div></AdminCard>
        <AdminCard className="p-5"><div className="flex items-center justify-between gap-3"><div><div className="admin-eyebrow">قیمت‌گذاری و نرخ‌ها</div><h2 className="mt-1 text-base font-black text-white">نرخ و زمان‌بندی قیمت</h2></div><Gauge size={19} className="text-emerald-300" /></div><div className="mt-4 space-y-3 text-xs"><div className="flex items-center justify-between gap-3"><span className="text-brand-m_khonsa">سرویس نرخ ارز</span><AdminBadge tone={health.rateConfigured ? "success" : "warning"}>{health.rateConfigured ? "پیکربندی‌شده" : "یافت نشد"}</AdminBadge></div><div className="flex items-center justify-between gap-3"><span className="text-brand-m_khonsa">فاصله همگام‌سازی</span><strong className="text-white">{health.rateIntervalHours} ساعت</strong></div><div className="flex items-center justify-between gap-3"><span className="text-brand-m_khonsa">آخرین همگام‌سازی</span><strong className="text-white" dir="ltr">{fmt(health.rateLastSyncAt)}</strong></div><div className="flex items-center justify-between gap-3"><span className="text-brand-m_khonsa">اجرای بعدی</span><strong className="text-white" dir="ltr">{fmt(health.rateNextRunAt)}</strong></div></div>{canRates && <button type="button" onClick={() => void run("rateSync")} disabled={!!busy} className="admin-button admin-button-primary mt-5"><Zap size={14} />{busy === "rateSync" ? "در حال اجرا..." : "همگام‌سازی نرخ‌ها"}</button>}</AdminCard>
        <AdminCard className="p-5"><div className="flex items-center justify-between gap-3"><div><div className="admin-eyebrow">زمان‌بندی</div><h2 className="mt-1 text-base font-black text-white">بازسازی قیمت‌ها</h2></div><TimerReset size={19} className="text-amber-300" /></div><div className="mt-4 space-y-2 text-xs"><div className="flex justify-between"><span className="text-brand-m_khonsa">وضعیت</span><AdminBadge tone={badgeTone(health.schedulerStatus)}>{statusLabel[health.schedulerStatus] || health.schedulerStatus}</AdminBadge></div><div className="flex justify-between gap-4"><span className="text-brand-m_khonsa">پیشرفت</span><strong className="text-white text-left" dir="ltr">{health.schedulerProgress}</strong></div><div className="flex justify-between"><span className="text-brand-m_khonsa">درخواست در انتظار</span><span className="text-white">{health.schedulerPendingRequest ? "بله" : "خیر"}</span></div></div>{canScheduler && <button type="button" onClick={() => void run("pricingRebuild", { currencies: [] })} disabled={!!busy} className="admin-button admin-button-muted mt-5"><Wrench size={14} />{busy === "pricingRebuild" ? "در حال قرار دادن در صف..." : "بازسازی کامل قیمت‌ها"}</button>}</AdminCard>
        <AdminCard className="p-5"><div className="flex items-center justify-between gap-3"><div><div className="admin-eyebrow">پاک‌سازی کش</div><h2 className="mt-1 text-base font-black text-white">پاک‌سازی کش</h2></div><RefreshCw size={19} className="text-brand-blue" /></div><div className="mt-4 grid grid-cols-3 gap-2 text-center text-xs"><div className="rounded-xl border border-white/[.06] p-3"><div className="text-[10px] text-brand-m_khonsa">Pending</div><strong className="text-white">{health.revalidationPending}</strong></div><div className="rounded-xl border border-white/[.06] p-3"><div className="text-[10px] text-brand-m_khonsa">Claimed</div><strong className="text-white">{health.revalidationClaimed}</strong></div><div className="rounded-xl border border-white/[.06] p-3"><div className="text-[10px] text-brand-m_khonsa">Failed</div><strong className="text-white">{health.revalidationFailed}</strong></div></div><div className="mt-3 text-[10px] text-brand-m_khonsa">Table: {health.revalidationTableReady ? "آماده" : "حالت جایگزین"} · Action: {health.revalidationScheduled ? "زمان‌بندی شده" : "زمان‌بندی نشده"}</div>{canRevalidate && <button type="button" onClick={() => void run("revalidation")} disabled={!!busy} className="admin-button admin-button-muted mt-5"><RefreshCw size={14} />{busy === "revalidation" ? "در حال قرار دادن در صف..." : "اجرای Revalidation"}</button>}</AdminCard>
      </div>
      <AdminCard className="mt-4 overflow-hidden"><div className="flex items-center justify-between border-b border-white/[.06] p-5"><div><div className="admin-eyebrow">کارهای ناموفق</div><h2 className="mt-1 text-base font-black text-white">کارهای ناموفق زمان‌بندی سیستم</h2></div><AlertTriangle size={19} className={failedJobs.length ? "text-amber-300" : "text-emerald-300"} /></div>{failedJobs.length === 0 ? <AdminEmpty title="کار ناموفقی برای هوک‌های BTL ثبت نشده است." /> : <div className="overflow-x-auto"><table className="w-full text-right text-xs"><thead><tr className="border-b border-white/[.06] text-brand-m_khonsa"><th className="px-5 py-3">شناسه</th><th>Hook</th><th>تلاش‌ها</th><th>آخرین تلاش</th><th className="px-5"></th></tr></thead><tbody>{failedJobs.map(job => <tr key={job.actionId} className="border-b border-white/[.04]"><td className="px-5 py-3 font-bold text-white">#{job.actionId}</td><td className="py-3 font-mono text-[11px] text-brand-m_khonsa" dir="ltr">{job.hook}</td><td className="py-3 text-white">{job.attempts}</td><td className="py-3 text-brand-m_khonsa" dir="ltr">{fmt(job.lastAttemptAt)}</td><td className="px-5 py-3 text-left">{canScheduler && <button type="button" onClick={() => void run("retryFailedJob", { actionId: job.actionId })} disabled={!!busy} className="admin-button admin-button-muted"><RotateCcw size={13} />تلاش مجدد</button>}</td></tr>)}</tbody></table></div>}</AdminCard>
      <div className="mt-4 grid gap-3 sm:grid-cols-3"><AdminStatCard label="زمان‌بندی سیستم" value={health.actionSchedulerAvailable ? "فعال" : "غیرفعال"} helper={`${health.actionSchedulerRunning} در حال اجرا · ${health.actionSchedulerOverdue} عقب‌افتاده`} tone={health.actionSchedulerAvailable ? "success" : "warning"} /><AdminStatCard label="جدول حسابرسی" value={health.auditTableReady ? "آماده" : "یافت نشد"} helper={`${health.auditEntries.toLocaleString("fa-IR")} رویداد`} tone={health.auditTableReady ? "success" : "warning"} /><AdminStatCard label="اقدام بعدی BTL" value={fmt(health.nextActionAt)} helper="زمان‌بندی سیستم" tone="info" /></div>
    </>}
  </AdminPage>;
}
