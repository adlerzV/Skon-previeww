...
export default function AdminAccountSettings(){
 ...
 return <AdminPage className="max-w-[1080px]">
  ...
  {error&&<div className="mb-4 rounded-xl border border-red-400/20 bg-red-500/5 p-4 text-xs text-red-300">{error}</div>}
  <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
   <AdminCard className="overflow-hidden"><div className="flex items-center gap-3 border-b border-white/[.06] p-4"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-brand-blue/10 text-brand-blue"><UserRound size={17}/></span><div><h2 className="text-sm font-black text-white">اطلاعات پروفایل</h2><p className="mt-1 text-[10px] text-brand-m_khonsa">نام و تصویر حساب مدیریت</p></div></div><div className="p-4">{!data?<AdminEmpty title="اطلاعات حساب در حال بارگذاری است."/>:<><AvatarPicker currentAvatarId={data.user.avatarId} currentAvatarUrl={data.user.avatarUrl} name={data.user.name} isStaff={data.user.isStaff}/><div className="mt-5 border-t border-white/[.06] pt-5"><ProfileEditForm name={data.user.name} email={data.user.email}/></div></>}</div></AdminCard>
   <AdminCard className="overflow-hidden"><div className="flex items-center gap-3 border-b border-white/[.06] p-4"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-emerald-400/10 text-emerald-300"><ShieldCheck size={17}/></span><div><h2 className="text-sm font-black text-white">امنیت</h2><p className="mt-1 text-[10px] text-brand-m_khonsa">رمز عبور و دسترسی حساب</p></div></div><div className="p-4">{!data?<AdminEmpty title="امنیت حساب در حال بارگذاری است."/>:<SetPasswordForm hasManualPassword={data.user.hasManualPassword}/>}</div></AdminCard>
  </div>
  <AdminCard className="mt-4 overflow-hidden"><div className="flex items-center gap-3 border-b border-white/[.06] p-4"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-white/[.04] text-brand-m_khonsa"><MonitorSmartphone size={17}/></span><div><h2 className="text-sm font-black text-white">نشست‌های فعال</h2><p className="mt-1 text-[10px] text-brand-m_khonsa">دستگاه‌هایی که به این حساب متصل‌اند.</p></div></div><div className="p-4">{!data?<AdminEmpty title="نشست‌ها در حال بارگذاری است."/>:<SessionsList sessions={data.sessions} currentSessionId={data.currentSessionId}/>}</div></AdminCard>
